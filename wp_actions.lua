...
group_waypoints.allow_waypoint_created(
	function(event)
		return pm_shim.player_can_see_group(event.player_id, event.waypoint.groupid)
	end
)

group_waypoints.allow_waypoint_updated(
	function(event)
		if event.player_id == event.waypoint.creator then
			return pm_shim.player_can_see_group(event.player_id, event.waypoint.groupid)
		else
			return pm_shim.player_can_modify_group(event.player_id, event.waypoint.groupid)
		end
	end
)
